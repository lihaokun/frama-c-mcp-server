...
fn main() {
    ...
    // Step 1: Compile OCaml code with Frama-C kernel into a complete object file
    // Need frama-c.kernel for the main API, frama-c.init.cmdline for cmdline init,
    // and dune-site.linker for dune plugin loading support.
    let ocaml_obj = out_dir.join("ocaml_lib.o");
    let status = Command::new("ocamlfind")
        .args([
            "ocamlopt",
            "-package",
            "frama-c.kernel,frama-c.init.cmdline,dune-site.linker",
            "-linkpkg",
            "-thread",
            "-cclib",
    ...
    let obj_size = std::fs::metadata(&ocaml_obj).unwrap().len();
    eprintln!("OCaml object size: {} bytes ({:.1} MB)", obj_size, obj_size as f64 / 1_048_576.0);
    ...
    // System libraries needed by OCaml runtime
